...
{{--                                        <option class="mx-5 col-6 col-sm-4 px-5" selected>{{ $this_section->title }}</option>--}}
{{--                                    @else--}}
{{--                                        <option class="mx-5  col-6 col-sm-4 px-5">{{ $this_section->title }}</option>--}}
{{--                                    @endif--}}
{{--                                @endforeach--}}
{{--                            --}}{{-- </optgroup> --}}
{{--                            @endif--}}
...
{{--                    </select>--}}
                     ...
                     <div class="dropdown-menu w-25" id="drop" aria-labelledby="dropdownMenuLink" style="max-height: 500; overflow: auto;">
                        @foreach ($categories as $category)
                            @if ($category->title == $section->category->title)
                                <a class="dropdown-item pl-3" href="javascript:void(0);">
                                    <div class="bg-success rounded mt-2 py-1 pl-1 val">{{ $category->title }}</div>
                                </a>
                            @else
                                <a class="dropdown-item pl-3 val" href="#">{{ $category->title }}</a>
                            @endif
                            @if (count($category->sections) > 0)
                                @foreach ($category->sections as $this_section)
                                    @if ($this_section->id == $section->id)
                                        <a class="dropdown-item pl-4" href="javascript:void(0);">
                                            <div class="bg-success rounded py-1 pl-1 val">{{ $this_section->title }}</div>
                                        </a>
                                    @else
                                        ...
                                        @foreach ($this_section->children as $child)
                                            @if ($child->parent_id == $section->id)
                                                <a class="dropdown-item pl-5" href="javascript:void(0);">
                                                    <div class="bg-success rounded py-1 pl-1 val">{{ $child->title }}</div>
                                                </a>
                                            @endif
                                        @endforeach
                                    @endif
                                        <a class="dropdown-item pl-4 val" href="javascript:void(0);">{{ $this_section->title }}</a>
                                    @endif
                                @endforeach
                            @endif
                        @endforeach
